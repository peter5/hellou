...
public class SubMitLocation extends Activity {
	...
	private EditText content_CX, content_KF, content_KH, content_et_SW,
			content_et_XW;
	...
	final Handler handler = new Handler() {
		...
		public void handleMessage(Message msg) {
			if (msg.what == 1) {
				...
				Toast.makeText(SubMitLocation.this, "上传成功！", Toast.LENGTH_SHORT)
						.show();
				...
				progressDialog.dismiss();
				Toast.makeText(SubMitLocation.this, "上传失败！", Toast.LENGTH_SHORT)
						.show();
			}
		}
	};
		...
		btn_submit.setOnClickListener(new OnClickListener() {
			...
			public void onClick(View v) {
				if (!MyUtil.isNetworkAvailable(SubMitLocation.this)) {
					Toast.makeText(SubMitLocation.this, "网络未连接！",
							Toast.LENGTH_SHORT).show();
					return;
				}
				final String _content_cx = content_CX.getText().toString()
						.trim();
				final String _content_kf = content_KF.getText().toString()
						.trim();
				final String _content_kh = content_KH.getText().toString()
						.trim();
				...
				if (_content_cx.equals("")) {
					Toast.makeText(SubMitLocation.this, "CX不能为空！",
							Toast.LENGTH_SHORT).show();
					return;
				}
				if (_content_kf.equals("")) {
					Toast.makeText(SubMitLocation.this, "KF不能为空！",
							Toast.LENGTH_SHORT).show();
					return;
				}
				if (_content_kh.equals("")) {
					Toast.makeText(SubMitLocation.this, "KH不能为空！",
							Toast.LENGTH_SHORT).show();
					return;
				}
...
//				}
					...
					public void run() {
						try {
							...
							if (test == null || !test.exists()) {
								submit(_content_cx, _content_kf, _content_kh,
										_content_sw, _content_xw);
								log("test null");
							} else {
								upPicture(_content_cx, _content_kf,
										_content_kh, _content_sw, _content_xw);
								log("test exists");
							}
						} catch (Exception e) {
							e.printStackTrace();
		...
		paizhao.setOnClickListener(new OnClickListener() {
			...
			public void onClick(View v) {
				...
				File photoDir = new File(Environment
						.getExternalStorageDirectory().getPath() + "/myImage1/");
				if (!sdStatus.equals(Environment.MEDIA_MOUNTED)) { // 检测sd是否可用
					Toast.makeText(SubMitLocation.this, "SD卡不存在",
							Toast.LENGTH_SHORT).show();
					return;
				}
				...
				if (!photoDir.exists()) {
					boolean b = photoDir.mkdirs();
					log("photoDir.mkdirs() is " + b);
					if (!b) {
						return;
					}
				}
				test = new File(photoDir, "photo_" + System.currentTimeMillis()
						+ ".jpg");
	...
	private void initlayout() {
		...
		content_CX.setFilters(new InputFilter[] { new InputFilter.LengthFilter(
				20) });
		content_KF.setFilters(new InputFilter[] { new InputFilter.LengthFilter(
				20) });
		content_CX.setFilters(new InputFilter[] { new InputFilter.LengthFilter(
				20) });
		content_et_SW
	...
	protected void onActivityResult(int requestCode, int resultCode, Intent data) {
		...
		if (requestCode == 0) {
			...
			if (resultCode == Activity.RESULT_OK) {
				...
				iv_pic.setImageBitmap(MyUtil.makeImg(dm.widthPixels - 100, 300,
	...
	void upPicture(final String _content_cx, final String _content_kf,
			final String _content_kh, final String _content_sw,
			final String _content_xw) {
			...
			new Thread() {
				...
				public void run() {
					try {
						...
						resultStr = JsonParser.UpLoadRes(
								SystemConfig.Location_URL,
								"{'uploadfile':{'userName':'"
						...
						if (resultStr.equals("")) {
							...
							if (result.equals("ok")) {
								submit2(_content_cx, _content_kf, _content_kh,
										_content_sw, _content_xw);
	...
	private void submit2(String _content_cx, String _content_kf,
			String _content_kh, String _content_sw, String _content_xw) {
		String resultStr = "";
		JSONObject resultJSON;
		...
		try {
			msgStr = "{'submitLocation':{'userName':'" + SystemConfig.userName
					+ "','forenoon':'" + _content_sw + "','afternoon':'"
					+ _content_xw + "','CX':'" + _content_cx + "','KF':'"
					+ _content_kf + "','KH':'" + _content_kh
					+ "','photo_name':'" + test.getName() + "','yyy':'"
					+ myLocation.getLatitude() + "','xxx':'"
					+ myLocation.getLongitude() + "'},'token':'"
					+ SystemConfig.token + "'}";
			resultStr = JsonParser.getResponse(SystemConfig.Location_URL,
					msgStr);
	...
	private void submit(String _content_cx, String _content_kf,
			String _content_kh, String _content_sw, String _content_xw) {
		String resultStr = "";
		JSONObject resultJSON;
		...
		try {
			msgStr = "{'submitLocation':{'userName':'" + SystemConfig.userName
					...
					+ _content_xw + "','CX':'" + _content_cx + "','KF':'"
					+ _content_kf + "','KH':'" + _content_kh
					+ "','photo_name':'','yyy':'" + myLocation.getLatitude()
					+ "','xxx':'" + myLocation.getLongitude() + "'},'token':'"
					+ SystemConfig.token + "'}";
			resultStr = JsonParser.getResponse(SystemConfig.Location_URL,
					msgStr);
			log("resultStr is  " + resultStr);
